...
def compute_dis_mat_numba(location):
    ...
    for i in prange(num_city):
        for j in prange(num_city):
            ...
            dis_mat[i, j] = np.sqrt(dx * dx + dy * dy)
    return dis_mat
...
class GA(object):
    ...
    def update_probabilities(self, scores):
        self.avg_fitness = np.mean(scores)
        self.max_fitness = np.max(scores)
        self.min_fitness = np.min(scores)

        delta = self.max_fitness - self.avg_fitness
        if delta > 0:
            self.cross_prob = self.initial_cross_prob - 0.5 * delta / (self.max_fitness - self.min_fitness)
            self.mutate_prob = self.initial_mutate_prob + 0.5 * delta / (self.max_fitness - self.min_fitness)
        else:
            self.cross_prob = self.initial_cross_prob
            self.mutate_prob = self.initial_mutate_prob
    ...
    def run(self):
        ...
        for i in range(1, self.iteration + 1):
            ...
            print(f"Iteration {i}: Best Length = {best_len:.2f}")
...
def main():
    ...
    model = GA(num_city=data.shape[0], num_total=100, iteration=80, data=data)
